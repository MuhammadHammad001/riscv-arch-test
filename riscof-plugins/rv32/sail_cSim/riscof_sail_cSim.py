...
class sail_cSim(pluginTemplate):
    ...
    def runTests(self, testList, cgf_file=None, header_file= None):
        ...
        for file in testList:
            ...
            execute += self.sail_exe[self.xlen] + ' --enable-dirty-update -i -v --trace=step --pmp-count=16 --pmp-grain=0 --test-signature={0} {1} > {2}.log 2>&1;'.format(sig_file, elf, test_name)
		# to enable pointer masking extension use --enable-zpm
		# to enable the hardware pte update for d and a bit use --enable-dirty-update
            cov_str = ' '
